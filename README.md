...
- **Rôles & permissions** — 4 rôles (`super_admin`, `admin`, `moderator`, `user`), seedés par `RolesPermissionsSeeder`, qui nettoie aussi les rôles/permissions par défaut devenus obsolètes (ex. l'ancien rôle `visitor`) d'une base déjà seedée.
- **Comptes** — le seeder de comptes (`AccountsSeeder`) bootstrap le compte Super Admin avec un mot de passe aléatoire (`secrets.token_urlsafe(24)`), affiché **une seule fois** dans les logs, jamais persisté en clair ; ce compte est verrouillé sur la page de changement de mot de passe tant qu'il ne l'a pas modifié. Il seede aussi un compte de debug (uniquement si `ENV_PROD=false`) — ce seeder n'est actuellement invoqué **que** par le point d'entrée `python app.py` en développement (`if __name__ == '__main__'`), pas depuis `entrypoint.sh`/`init_db.py`.
...
- **RBAC** : 4 rôles (`super_admin`, `admin`, `moderator`, `user`) et permissions nommées définies dans [`flask/permissions.py`](flask/permissions.py), appliquées via les décorateurs `@login_required` / `@require_permission("...")` (`flask/utils/decorators.py`). Seul `super_admin` peut gérer la définition des rôles eux-mêmes (`view_roles`/`create_role`/`edit_role`/`delete_role`) ; `admin` gère les comptes utilisateurs mais pas les rôles. Les actions sensibles sur des comptes `admin`/`super_admin` (changement de rôle, suspension, suppression) restent en plus protégées par des comparaisons de `role_name` codées en dur dans `admin/services.py`, et par `enforce_password_change` dans `app.py`.
